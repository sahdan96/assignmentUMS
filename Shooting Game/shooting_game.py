...
def main():
    ...
    pygame.display.set_caption('PUBS')
    pygame.mouse.set_visible(1)

# Create the background which will scroll and loop over a set of different
# size stars
    background = pygame.Surface((800, 2000))
    background = background.convert()
    background.fill((0, 0, 0))
    backgroundLoc = 1400
    finalStars = deque()
    for y in range(0, 1400, 30):
        size = random.randint(2, 5)
        x = random.randint(0, 800 - size)
        if y <= 600:
            finalStars.appendleft((x, y+1400, size))
        pygame.draw.rect(
...
# Prepare game objects
    ...
    selectText = font.render('*', 1, YELLOW)
    ...
    # Event Handling
        for event in pygame.event.get():
            ...
            ship.rect.x = last_pos[0]
            ship.rect.y = last_pos[1]
    ...
    # Update text overlays
        ...
        leftText = font.render("Aliens Left: " + str(aliensLeftThisWave),
                               1, YELLOW)
        scoreText = font.render("Score: " + str(score), 1, YELLOW)
        bombText = font.render("Shockwave[B]: " + str(bombsHeld), 1, RED)
    ...
    # Detertmine when to move to next wave
        if aliensLeftThisWave <= 0:
            if betweenWaveCount > 0:
                betweenWaveCount -= 1
                nextWaveText = font2.render(
                    'Wave ' + str(wave + 1) + ' in', 1, CYAN)
                nextWaveNum = font2.render(
                    str((betweenWaveCount // clockTime) + 1), 1, CYAN)
